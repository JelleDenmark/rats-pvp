...
import { UNIT_DEFS, tierAttackMultiplier, tierHealthMultiplier, reviveHpForTier, blockHitsForTier, poisonStacksForTier, cellarCoilChargeCapForTier, backlineTargetsForTier } from './data/units';
  ...
  const applyEffect = (source: BattleUnit, index: number, removed: boolean): void => {
    ...
    switch (effect.kind) {
      ...
      case 'backlineDamage': {
        ...
        // lands. Per-hit damage is deliberately the unit's FLAT base attack
        // — own def's base stat plus its own relics and (for horde units)
        // the team-attack pool, but with `tierAttackMultiplier` left out —
        // see `backlineTargetsForTier`'s doc comment for why stacking the
        // exponential 1x/3x/9x attack curve on top of a growing target count
        // was rejected as an AOE-nuke risk.
