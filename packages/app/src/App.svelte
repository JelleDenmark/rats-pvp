<script lang="ts">
  ...
  function abilitySentence(def: UnitDef | undefined, side: 'horde' | 'enemy' = 'horde'): string {
    ...
    if (e.kind === 'backlineDamage') {
      ...
      return `At the start of every wave, if not at the front, strikes the first ${foe} for ${base} (★2 hits the first ${targets(2)} for ${base} each · ★3 hits the first ${targets(3)} for ${base} each) — separate hits, landed before that wave's clashing even begins, with no retaliation. At the front, it just fights normally.`;
    }
    let what = '';
    switch (e.kind) {
      case 'summon': {
